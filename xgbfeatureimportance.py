...
for rs in R:
    ...
    model = xgboost.XGBRegressor(random_state=rs,n_estimators=100, learning_rate=0.08, gamma=0, subsample=0.75,colsample_bytree=1, max_depth=7)
...
#    #plt.savefig('resultsremoverows.png')
#    plt.savefig('Graph/resultsxgboost'+rsstr+'.png')
#    plt.show()
...
#    plt.legend()
#    plt.savefig('Graph/xgboost'+rsstr+'.png')
#    plt.show()
    ...
    plt.bar(train_cols, model.feature_importances_)
    plt.xticks(rotation=90)
    plt.ylabel('Feature Importance (%)')
    plt.xlabel('Features')
    plt.legend()
    plt.savefig('Graph/xgboostfeatureimportance'+rsstr+'.png')
    plt.show()
